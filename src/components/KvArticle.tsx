...
import { buildKvFaqs, faqJsonLd } from "@/lib/faq";
...
/**
 * The full KV article body, shared by the public /kv/[slug] page and the
 * noindex /vorschau/kv/[slug] staging preview. `preview` swaps the base path so
 * canonical/links stay internally consistent on staging.
 */
export function KvArticle({ kv, preview = false }: { kv: CollectiveAgreement; preview?: boolean }) {
  const base = preview ? "/vorschau" : "";
    ...
    <article className="space-y-10">
      <Breadcrumbs
        items={[
          ...
          { name: kv.shortName ?? kv.name, href: `${base}/kv/${kv.slug}` },
        ]}
      />
      ...
      {unverified && (
        <div role="alert" className="rounded-lg border border-warn/40 bg-warn-soft p-4 text-sm text-warn">
          ...
          <Link href="/korrektur-melden" className="underline">Fehler melden</Link>.
        </div>
      )}
          ...
          {kv.groups.map((g) => (
            <li key={g.code} className="rounded-lg border border-surface-border bg-surface p-4">
              <h3 className="font-semibold">
                <Link href={`${base}/kv/${kv.slug}/verwendungsgruppe/${encodeURIComponent(g.code)}`} className="hover:text-brand-600">
      ...
      {kv.allowances && kv.allowances.length > 0 && (
        <section aria-labelledby="zulagen" className="space-y-3">
          ...
          <ul className="space-y-1 text-sm text-ink-soft">
            {kv.allowances.map((z) => (
              <li key={z.name}>
                {z.name}: <strong>{formatEUR(z.amountEUR)}</strong> / {z.basis} (ab {formatDate(z.validFrom)})
              </li>
            ))}
          </ul>
        </section>
        ...
        <ul className="space-y-2 text-sm">
          {history.map((p) => (
            <li key={p.id} className="flex flex-wrap items-center gap-2">
              ...
              <span>ab {formatDate(p.validFrom)}{p.validTo ? ` – ${formatDate(p.validTo)}` : ""}</span>
          ...
          <ul className="space-y-2">
            {related.map((r) => (
              <li key={r.id} className="flex flex-wrap items-center gap-3 text-sm">
                <Link href={`${base}/kv/${r.slug}`} className="inline-block rounded-full border border-surface-border bg-surface px-3 py-1.5 hover:border-brand-500">
                  {r.shortName ?? r.name}
                </Link>
                {r.employeeType === kv.employeeType && (
                  <Link href={`/vergleich/${comparisonSlug(kv.slug, r.slug)}`} className="text-brand-600 hover:underline">
                    vergleichen →
                  </Link>
                )}
              </li>
      ...
      {faqs.length > 0 && (
        <section aria-labelledby="faq" className="space-y-3">
          <h2 id="faq" className="text-xl font-semibold">Häufige Fragen</h2>
          <dl className="space-y-3">
            {faqs.map((f) => (
              <div key={f.q} className="rounded-lg border border-surface-border bg-surface p-4">
      ...
      <section aria-labelledby="quellen" className="space-y-3">
        ...
        <ul className="space-y-1 text-sm">
          {kv.sourceUrls.map((u) => (
            <li key={u}>
              <a href={u} target="_blank" rel="noopener noreferrer nofollow" className="text-brand-600 hover:underline">{u}</a>
            </li>
          ))}
        </ul>
